...
const getSchemaJson = (command, name, collection) => {
    const schematicJsonFilePath = (0, get_schema_path_1.getSchemaPath)();
    if (!schematicJsonFilePath) {
        let error = `Unable to find schema for ${command} ${collection}:${name}`;
